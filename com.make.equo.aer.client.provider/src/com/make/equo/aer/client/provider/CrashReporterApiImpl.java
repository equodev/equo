...
@Component
public class CrashReporterApiImpl implements ICrashReporterApi{
	...
	@Activate
	public void start() {
		System.out.println("Initializing CrashReporter Client provider...");
	...
	public void logCrash(JsonObject segmentation, String type, String message) {
		switch(type) {
		case TYPE_INFO:
			equoLoggingService.logInfo(message, segmentation);
			break;
		case TYPE_WARNING:
			equoLoggingService.logWarning(message, segmentation);
			break;
		case TYPE_ERROR:
			equoLoggingService.logError(message, segmentation);
			break;
		}
	}
	...
	public void logCrash(String type, String message) {
		switch(type) {
		case TYPE_INFO:
			equoLoggingService.logInfo(message);
			break;
		case TYPE_WARNING:
			equoLoggingService.logWarning(message);
			break;
		case TYPE_ERROR:
			equoLoggingService.logError(message);
			break;
		}
	...
	private class LoggingEventPayloadRunnable implements JsonPayloadEquoRunnable {
		...
		public void run(JsonObject payload) {
			System.out.println("Log Event json payload is " + payload);
			...
			String typeLog = typeJsonElement.getAsString();
			...
			if (segmentationJsonObject == null) {
				logCrash(typeLog, message);
			} else {
				logCrash(segmentationJsonObject, typeLog, message);
			}
		}
