...
public class ToolbarRendererEventTest extends AbstractBuilderTest {
	...
	public void toolbarRenderer_Should_Store_ClickEventHandler_in_WSocket_EventsHash() throws Exception {
		...
		assertThat(messageService).isNotNull().extracting("eventHandlers")
				.asInstanceOf(InstanceOfAssertFactories.map(String.class, String.class)).isNotNull()
		...
		EmptyClient.create(new URI("ws://localhost:" + messageService.getPort()))
				.singleMessage("{\"action\":\"" + "Toolbar" + toHexString(toolbar.hashCode()) + "_itemClicked"
						+ "\",\"params\":{\"toolBarElementId\":\"" + toolItem.getElementId() + "\",\"commandId\":\""
						+ toolItem.getCommand().getElementId() + "\",\"isAnEquoModelElement\":\"true\"}}")
				.connect();
	...
	private void initializeCommandAndHandlerAddons() {
		...
		this.injector.getHandlerAddon().postConstruct(modelTestingConfigurator.getMainApplication(),
				this.injector.getModelService());
	}
