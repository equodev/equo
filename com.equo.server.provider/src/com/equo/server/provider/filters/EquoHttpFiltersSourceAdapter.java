...
public class EquoHttpFiltersSourceAdapter extends HttpFiltersSourceAdapter {
	...
	private Optional<String> getRequestedUrl(HttpRequest originalRequest) {
		return Stream.concat(contributionRequestHandler.getContributionProxiedUris().stream(), proxiedUrls.stream())
				.filter(url -> containsHeader(url, originalRequest)).findFirst();
	}
